$(function () {
  ...
  $.ajax(settings).done(function (response) {
    ...
    if (response.length) {
      response[1].forEach((el, index) => {
        const date = new Date(el).getDate();
        tickpoint.push(date);
        const lastDate = new Date(response[1][index - 1]);
        if (index !== 0 && date - lastDate.getDate() > 1) {
    ...
    Highcharts.chart("container_bhp_1", {
      chart: {
        // spacingTop: 0,
        // width: 375,
        height: 280,
      ...
      tooltip: {
        ...
        formatter: function () {
          ...
          // var date = new Date(ts).toLocaleString("en-US", {timeZone: "Asia/Calcutta"});
      ...
      xAxis: {
        ...
        breaks: [
          ...
          {
            from: Date.UTC(2023, 1, 21, 21, 0o1),
